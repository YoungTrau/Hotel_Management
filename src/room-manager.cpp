...
bool RoomManager::EraseAvailableRoom(const int& number){
    ...
    if(guess == NULL || guess->getCheckout() != "No information"){
...
bool RoomManager::RoomInfo(const int& room)
{
    ...
    if(guess != NULL){
        ...
        UI::showMessage("Phone: " + guess->getPhone());
        UI::showMessage("Checkin - checkout history");
        UI::showMessage("Time\tcheckin/out");
