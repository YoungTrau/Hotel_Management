...
vector<string> Negative_content = {"Need to improve payment processing speed.",
                                    "Service is okay, nothing special.",
                                    "Had to wait too long to be served.",
                                    };
vector<string> Positve_content = {"Very friendly staff.",
                                  "Clean and tidy bathroom.",
                                  "Meals are served with a variety of dishes."};
...
feedback_dtype GuessEmployee :: GenerateRandomFeedback(){
    ...
    switch(type){
        ...
        case NEGATIVE:
            ...
            break;
        case NONE_FEEDBACK:
            feedback_result[NONE_FEEDBACK] = "No comment";
        default:
            break;
    }
...
void GuessEmployee::ServiceFeedbackInfo(const Guess& guess){
    ...
    auto service_description = [guess](){
        ...
        return string("No service's used");
    };

    UI::showMessage("Service used");
    UI::showMessage("Name\t\t\tPrices\t\t\tQuantity");
    UI::showMessage(guess.getService() + "\t\t|" + to_string(price) + "\t\t\t|" +  to_string(quantity));
    UI::showMessage("Time in: " + to_string(day_count));
    UI::showMessage("Total service cost: " + to_string(raw_cost) + " $");
    UI::showMessage("VAT: 10%");
    UI::showMessage("Feedback: " + service_description());    
    UI::showMessage("Total amount: " + to_string(final_cost) + " $");
}
Guess* GuessEmployee::GuessInfo(const string& phone){
    Guess* guess = findRoombyPhone(phone);
    if(guess != NULL){
        UI::showMessage("Customer information");
        UI::showMessage("Name: " + guess->getName());
        UI::showMessage("Phone number: " + guess->getPhone());
        UI::showMessage("Room: " + to_string(guess->getRoom()));
        UI::showMessage("Check In time: " + guess->getCheckin());
...
bool GuessEmployee::ShowListGuess(){
    ...
    for(auto& guess : list_guess){
        UI::showMessage("Room: " + to_string(guess.getRoom()) + 
                        ",Name: " + guess.getName() + 
                        ",Phone: " + guess.getPhone() + 
                        ",Check in: " + guess.getCheckin() + 
                        ",Check out: " + guess.getCheckout());
    }
    return SUCCESS;
}
